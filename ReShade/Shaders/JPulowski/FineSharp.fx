...
namespace JPulowski
{
	...
	{
		return float3x3(
			float3(Kr, 1.0 - Kr - Kb, Kb),
			float3(-Kr, Kr + Kb - 1.0, 1.0 - Kb) / (2.0 * (1.0 - Kb)),
			float3(1.0 - Kr, Kr + Kb - 1.0, -Kb) / (2.0 * (1.0 - Kr)));
	}
	float3x3 YUVtoRGB(float Kb, float Kr) {
		return float3x3(
			float3(1.0, 0.0, 2.0 * (1.0 - Kr)),
			float3(Kb + Kr - 1.0, 2.0 * (1.0 - Kb) * Kb, 2 * Kr * (1.0 - Kr)) / (Kb + Kr - 1.0),
			float3(1.0, 2.0 * (1.0 - Kb), 0.0));
	}
...
technique FineSharp_Tech < enabled = RESHADE_START_ENABLED; toggle = FineSharp_ToggleKey; >
{
	pass ToYUV
	{
		VertexShader = ReShade::VS_PostProcess;
		PixelShader = JPulowski::PS_FineSharp_P0;
	}
	...
	{
		VertexShader = ReShade::VS_PostProcess;
		PixelShader = JPulowski::PS_FineSharp_P2;
	}
	pass RemoveGrain11
	{
		VertexShader = ReShade::VS_PostProcess;
		PixelShader = JPulowski::PS_FineSharp_P1;
	}
#elif (mode == 3)
	pass RemoveGrain4
	{
	...
	{
		VertexShader = ReShade::VS_PostProcess;
		PixelShader = JPulowski::PS_FineSharp_P3;
	}
	pass FineSharpB
	{
		VertexShader = ReShade::VS_PostProcess;
		PixelShader = JPulowski::PS_FineSharp_P4;
	}
	pass FineSharpC
	{
		VertexShader = ReShade::VS_PostProcess;
		PixelShader = JPulowski::PS_FineSharp_P5;
	}

	pass ToRGB
	{
		VertexShader = ReShade::VS_PostProcess;
		PixelShader = JPulowski::PS_FineSharp_P6;
	}
}
