	...
	float3 PS_ModePaint(float4 vpos : SV_POSITION, float2 texcoord : TEXCOORD0) : SV_TARGET
	{
		...
		for (int i = -PaintRadius; i <= PaintRadius; i++)
		{
			for (int j = -PaintRadius; j <= PaintRadius; j++)
			{
				...
				col.a   = round(dot(col.rgb, lumcoeff)) + 1.0;	// Store intensity in alpha channel and increase it by 1, so we can count values between 0.0 - 1.0
...
technique Paint_Tech < enabled = RESHADE_START_ENABLED; toggle = Paint_ToggleKey; >
{
#if (PaintMethod == 1)
	pass Kuwahara
	{
		VertexShader = ReShade::VS_PostProcess;
		PixelShader = JPulowski::PS_Kuwahara;
	}
#else
	pass kingericPaint
	{
		VertexShader = ReShade::VS_PostProcess;
		PixelShader = JPulowski::PS_ModePaint;
	}
#endif
}
