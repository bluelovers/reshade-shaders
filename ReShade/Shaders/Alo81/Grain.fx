...
{
	float noise = sin(dot(float3(tc.x, tc.y, ReShade::Timer), float3(12.9898, 78.233, 0.0025216))) * 43758.5453;
...
float4 GrainPass(float4 position : SV_Position, float2 texcoord : TEXCOORD) : SV_Target
{
	...
	float2 rotCoordsR = coordRot(texcoord, ReShade::Timer + rotOffset.x);
	float2 rot = rotCoordsR * ReShade::ScreenSize / (GrainSize).xx;
	float pNoise = pnoise3D(float3(rot.x, rot.y, 0.0));
	float3 noise = float3(pNoise, pNoise, pNoise);

#if GrainColored
	float2 rotCoordsG = coordRot(texcoord, ReShade::Timer + rotOffset.y);
	float2 rotCoordsB = coordRot(texcoord, ReShade::Timer + rotOffset.z);
	noise.g = lerp(noise.r, pnoise3D(float3(rotCoordsG * ReShade::ScreenSize / (GrainSize).xx, 1.0)), GrainColorAmount);
	noise.b = lerp(noise.r, pnoise3D(float3(rotCoordsB * ReShade::ScreenSize / (GrainSize).xx, 2.0)), GrainColorAmount);
#endif
	...
	float luminance = lerp(0.0, dot(col, lumcoeff), GrainLuma);
	...
	col += noise * GrainPower;

	return float4(col, 1.0);
}
}

technique Grain_Tech < enabled = RESHADE_START_ENABLED; toggle = Grain_ToggleKey; >
{
	pass Grain_Pass
	{
		VertexShader = ReShade::VS_PostProcess;
		PixelShader = Alo81::GrainPass;
	}
}
