...
{
#if (sNoiseMode == 1)
	texture texnoise2 < source = "ReShade/Shaders/IDDQD/Textures/VHS_N1.jpg"; >
#elif (sNoiseMode == 2)
	texture texnoise2 < source = "ReShade/Shaders/IDDQD/Textures/VHS_N2.jpg"; >
#else
	texture texnoise2 < source = "ReShade/Shaders/IDDQD/Textures/VHS_N3.jpg"; >
#endif
	{
		Width = BUFFER_WIDTH;
...
#define t2 ReShade::Timer * 0.001
	...
	{
		float sampless = tex2D(SamNoise, float2(1.0, 2.0 * cos(t2)) * t2 * 8.0 + p * 1.0).x;
		sampless *= sampless;
		return sampless;
	}
	...
	float stripes(float2 uv)
	{
		...
		return ramp((uv.y * 4.0 + t2 / 2.0 + sin(t2 + sin(t2 * 0.63)) % 2.0), 0.5, 0.6) * noi;
	}
	...
	float3 getVideo(float2 uv)
	{
		...
		look.x = look.x + sin(look.y * 10.0 + t2) / 500.0 * onOff(4.0, 4.0, 0.3) * (1.0 + cos(t2 * 80.0)) * window;
		float vShift = 5.4 * onOff(2.0, 3.0, 0.9) * (sin(t2) * sin(t2 * 20.0) + (0.5 + 0.1 * sin(t2 * 200.0) * cos(t2)));
	...
	{
		return float2(uv.x, uv.y + sin(t2) * 0.02);
	}

	float noise2(float2 uv) {
		return clamp(tex2D(SamNoise, uv.xy + t2 * 6.0).r + tex2D(SamNoise, uv.xy - t2 * 4.0).g, 0.96, 1.0);
	}
	...
	{
		return float2(uv.x, uv.y + amount * rand);
	}
	...
	void PS_VHS(float4 vpos : SV_Position, float2 texcoord : TEXCOORD, out float4 origcolor : SV_Target0)
	{
		origcolor = tex2D(ReShade::BackBuffer, texcoord);
		origcolor = NTSCCodec(texcoord);
		origcolor = pow(origcolor, 0.5);
	}
	void PS_VHS3(float4 vpos : SV_Position, float2 texcoord : TEXCOORD, out float4 origcolor2 : SV_Target0)
	{
		float4 uv = 0.0;
		uv.xy = texcoord.xy;
		uv.w = 0.0;
		origcolor2 = tex2Dlod(ReShade::BackBuffer, uv);
		...
		origcolor2 += dot(nn(-uv.xy), 0.233);
	}
	void PS_VHS4(float4 vpos : SV_Position, float2 texcoord : TEXCOORD, out float4 origcolor3 : SV_Target0)
	{
		origcolor3 = tex2Dlod(ReShade::BackBuffer, float4(texcoord, 0, 0));

		texcoord = screenDistort(texcoord);
		float3 video = getVideo(texcoord);
		float vigAmt = 3.0 + 0.3 * sin(t2 + 5.0 * cos(t2 * 5.0));
		float vignette = (1.0 - vigAmt * (texcoord.y - 0.5) * (texcoord.y - 0.5)) * (1.0 - vigAmt * (texcoord.x - 0.5) * (texcoord.x - 0.5));

		//video += stripes(texcoord);
		video += noise(texcoord * 2.0) / 2.0;
		video *= vignette;

		origcolor3.xyz = video;
	}
	void PS_VHS5(float4 vpos : SV_Position, float2 texcoord : TEXCOORD, out float4 origcolor4 : SV_Target0)
	{
		origcolor4 = tex2D(ReShade::BackBuffer, texcoord);
	...
	void PS_VHS6(float4 vpos : SV_Position, float2 texcoord : TEXCOORD, out float4 OutGet3 : SV_Target0)
	{
		...
		OutGet3 = lerp(color, lerp(scanlineColor, slowscanColor, 0.5), 0.05) * vignette(texcoord) * noise2(texcoord);	
...
technique VHSReShade < enabled = RESHADE_START_ENABLED; toggle = VHS_ToggleKey; >
{
#if (bUseNTSCFilter == 1)
	pass NTSCFilter
	{
		VertexShader = ReShade::VS_PostProcess;
		PixelShader = IDDQD::PS_VHS;
	}
#endif
#if (bUseVCRDistort == 1)
	pass VCRDistort
